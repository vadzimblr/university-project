# Story Illustrator Frontend (Vue 3 + Vite)

Фронтенд для автоматической иллюстрации рассказа из PDF.
...
- Отдельный `Storybook mode` (reader-first): цельная страница чтения с последовательными панелями, увеличенным текстом и пагинацией как в книге.
...
- Границы сцен редактируются через drop-down распределения: начальные предложения можно отправить только в prev, конечные — только в next (середина не выдёргивается).
- Генерация изображений доступна только после полного approve границ (не осталось `pending` сцен).
