...
class InboxRepository:
    ...
    def save_event(
        ...
        """
        Saves an event to the inbox. Returns None if event with same idempotency_key already exists.
        """
        if idempotency_key:
            existing = (
    ...
    def get_unprocessed_events(self, limit: int = 100, max_retries: int = 5) -> List[InboxEvent]:
        """
        Gets unprocessed events with retry_count < max_retries.
        Uses with_for_update(skip_locked=True) to prevent concurrent processing.
        """
        return (
            self.db.query(InboxEvent)
            .filter(
    ...
    def mark_as_failed(self, event_id: UUID, error_message: str) -> bool:
        """
        Marks an event as permanently failed (after max retries).
        This moves the event to a "dead letter queue" state.
        """
        event = self.db.query(InboxEvent).filter(InboxEvent.id == event_id).first()
        if not event:
            return False
    ...
    def get_failed_events(self, max_retries: int = 3) -> List[InboxEvent]:
        """Gets events that have reached max retries but are not processed."""
        return (
            self.db.query(InboxEvent)
            .filter(
                InboxEvent.processed == False,
    ...
    def delete_processed_events(self, older_than_days: int = 7) -> int:
        """Deletes processed events older than specified days."""
