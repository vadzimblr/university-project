...
<template>
  <section class="comic-card space-y-4 bg-white p-4">
    <div class="rounded-lg border-2 border-slate-900 bg-amber-50 p-3 text-sm">
      <p class="font-black">Как пользоваться этой панелью</p>
      <ol class="mt-1 list-decimal pl-5 text-xs text-slate-700">
        <li>Выделите предложение в центре (оно станет жёлтым).</li>
        <li>Нажмите <b>Split по выделенному</b>, чтобы разделить сцену.</li>
        <li>Кнопками Start/End подвиньте границы на 1 предложение.</li>
      </ol>
    </div>
    ...
    <div class="flex items-center justify-between gap-2">
      <div>
        ...
        <p class="text-sm text-slate-600">Статус: {{ scene.status }} · Границы {{ scene.startIdx }}-{{ scene.endIdx }}</p>
      </div>
      <button
    ...
    <div class="space-y-2">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <p class="text-sm font-black">Работа с границами</p>
        <div class="flex items-center gap-2 text-xs">
          ...
          <span class="rounded border-2 border-slate-900 bg-slate-50 px-2 py-1">окно {{ chunkRangeLabel }}</span>
          <button class="rounded border-2 border-slate-900 bg-white px-2 py-1" :disabled="chunkStart === 0" @click="shiftChunk('prev')">← Окно</button>
          <button class="rounded border-2 border-slate-900 bg-white px-2 py-1" :disabled="chunkStart + chunkSize >= totalInside" @click="shiftChunk('next')">Окно →</button>
        </div>
      </div>
