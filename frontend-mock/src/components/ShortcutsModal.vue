<script setup lang="ts">
defineProps<{ open: boolean }>();
const emit = defineEmits<{ close: [] }>();
</script>

<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4" @click.self="emit('close')">
    <div class="w-full max-w-lg rounded-xl bg-white p-5">
      <h3 class="mb-3 text-lg font-semibold">Keyboard shortcuts</h3>
      <ul class="space-y-2 text-sm text-slate-700">
        <li><b>A</b> — approve / unapprove выбранную сцену</li>
        <li><b>G</b> — запустить генерацию approved сцен</li>
        <li><b>← / →</b> — переключение между сценами</li>
      </ul>
      <div class="mt-4 text-right">
        <button class="rounded-lg border border-slate-200 px-3 py-1.5" @click="emit('close')">Понятно</button>
      </div>
    </div>
  </div>
</template>
